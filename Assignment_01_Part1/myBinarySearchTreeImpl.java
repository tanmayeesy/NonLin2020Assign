...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		if (this.root == null || level < 1)  
			return 0;
		if (level == 1)
			return 1;
		else
			return this.my_right_tree().my_count_at_level(level - 1) + this.my_left_tree().my_count_at_level(level - 1);
	}
	...
	public boolean my_is_balanced(){
		...
		return (this.root == null) ||
				(this.my_left_tree().my_is_balanced() &&
						this.my_right_tree().my_is_balanced()&&
						Math.abs(this.my_left_tree().my_length() - this.my_right_tree().my_length()) <=1);
	}
	...
	//-------------------------------------------------------------------	
	public int my_find_node_at_level(T1 key, int level) {
		...
		int result = this.my_left_tree().my_find_node_at_level(key,level+1) ;

		if(result != 0)
			return result;

		result = this.my_right_tree().my_find_node_at_level(key,level+1) ;
